...
function Block({ type, content, imageUrls }: { type: string; content: Json; imageUrls: Map<string, string> }) {
  ...
  if (type === "image" && typeof record.imageId === "string") { const url = imageUrls.get(record.imageId); return url ? <img src={url} alt={typeof record.alt === "string" ? record.alt : "Article image"} className="max-h-[32rem] w-full object-contain" loading="lazy" /> : null; }
  return null;
}
