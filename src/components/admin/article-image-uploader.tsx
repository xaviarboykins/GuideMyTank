...
export function ArticleImageUploader({ articleId, nextOrder }: { articleId: string; nextOrder: number }) {
  ...
  async function upload(formData: FormData) {
    ...
    setPending(false);
    if (assignmentError) return setMessage("The image could not be attached to this article.");
    setMessage("Image uploaded and attached.");
    router.refresh();
  }

  return <form action={upload} className="grid gap-3 border border-border bg-background p-4 md:grid-cols-2">
    ...
    <label className="space-y-1"><span className="text-sm font-medium">Alt text</span><Input name="altText" /></label>
    ...
    <div className="md:col-span-2"><Button type="submit" disabled={pending}>{pending ? "Uploading…" : "Upload article image"}</Button>{message ? <p role="status" className="mt-2 text-sm">{message}</p> : null}</div>
  </form>;
}
